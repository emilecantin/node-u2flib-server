...
exports.finishRegistration = function(challenge, deviceResponse){
  ...
  if (!deviceResponse.clientData) { throw 'No clientData'; }
  ...
  if (!clientData.typ) { throw 'No clientData.typ'; }
  if (clientData.typ !== "navigator.id.finishEnrollment") { throw 'Wrong type'; }
  if (clientData.challenge !== challenge.challenge) { throw 'Wrong challenge'; }

  // Then, the "registrationData" part.
  if (!deviceResponse.registrationData) { throw 'No registrationData'; }

  var offset = 0
  var registrationData = new Buffer(deviceResponse.registrationData, 'base64');
  // Read reserved byte
  var reservedByte = registrationData.readInt8(offset++);
  if (reservedByte !== 0x05) { throw 'Wrong reserved byte'; }

  // Read public key
  var publicKey = registrationData.slice(offset, offset + PUBKEY_LEN);
  var tmpKey = pubKeyToPem(publicKey);
  if (!tmpKey) { throw 'Public key error'; }
...
exports.finishAuthentication = function(challenge, deviceResponse, deviceRegistration){
  // Validation
  if (!deviceResponse.keyHandle) { throw 'No keyHandle'; }
  if (!deviceResponse.clientData) { throw 'No clientData'; }
  if (!deviceResponse.signatureData) { throw 'No signatureData'; }
  if (deviceResponse.keyHandle !== challenge.keyHandle) { throw 'Wrong keyHandle'; }

  var rawClientData = (new Buffer(deviceResponse.clientData, 'base64')).toString();
  var clientData = JSON.parse(rawClientData);
  if (!clientData.typ) { throw 'No clientData.typ'; }
  if (clientData.typ !== "navigator.id.getAssertion") { throw 'Wrong type'; }
  if (clientData.challenge !== challenge.challenge) { throw 'Wrong challenge'; }
  ...
  if(!is_valid) { throw 'Signature not valid'; }
